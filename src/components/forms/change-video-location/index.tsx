...
const ChangeVideoLocation = ({
  ...
  const folder = folders.find((f: { id: string }) => f.id === currentFolder)
  const workspace = workspaces.find((f: { id: string }) => f.id === currentWorkSpace)
    ...
    >
      ...
      <div className="flex flex-col gap-y-5 p-5 border-[1px] rounded-xl">
        ...
        <Label className="flex-col gap-y-2 flex">
          ...
          >
            {workspaces.map((space: { id: string; name: string }) => (
              <option
                key={space.id}
